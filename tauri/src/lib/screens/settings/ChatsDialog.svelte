<script lang="ts">
  ...
  const { trashEnabled, retentionDays, onConfirm, onDismiss }: Props = $props();
  ...
  const valid = $derived(/^\d+$/.test(days.trim()) && parsed >= MIN_DAYS);
  const error = $derived(days.trim() === "" || valid ? null : t("RETENTION_DAYS_ERROR", MIN_DAYS));
</script>
