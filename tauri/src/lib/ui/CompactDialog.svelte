...
<Dialog.Root open onOpenChange={(open) => !open && onDismiss()}>
  <Dialog.Portal>
    ...
    <Dialog.Content
      class="menu-surface fixed top-1/2 left-1/2 z-50 flex max-h-[85vh] w-max min-w-80 max-w-[min(32rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 flex-col rounded-lg border border-outline-variant bg-surface p-5 shadow-xl"
    >
      <div class="flex items-start gap-2">
        <div class="min-w-0 flex-1">
          <Dialog.Title class="truncate text-dialog-title">{title}</Dialog.Title>
          {#if description}
            <Dialog.Description class="mt-1 text-body-md text-on-surface-variant">
              {description}
            </Dialog.Description>
          {/if}
        </div>
